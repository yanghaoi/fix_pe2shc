...
void scan_no_reloc_risks(BYTE *my_exe, size_t exe_size)
{
	size_t risks_found = 0;

	//--- Check Load Config (GS, SEH, CFG) ---
	BYTE* ld_cfg = peconv::get_load_config_ptr(my_exe, exe_size);
	if (ld_cfg) {
		peconv::t_load_config_ver ver = peconv::get_load_config_version(my_exe, exe_size, ld_cfg);
		bool is64 = peconv::is64bit(my_exe);

		if (is64) {
			// x64: cast to W10 struct (largest, all fields present)
			auto cfg = (peconv::IMAGE_LOAD_CONFIG_DIR64_W10*)ld_cfg;
			if (cfg->Size < sizeof(*cfg)) {
				// fallback: try W8
				auto cfg8 = (peconv::IMAGE_LOAD_CONFIG_DIR64_W8*)ld_cfg;
				if (cfg8->SecurityCookie) {
					std::cout << "[WARN] /GS security cookie detected - may fail in no-reloc fallback mode" << std::endl; risks_found++;
				}
				if (cfg8->SEHandlerTable && cfg8->SEHandlerCount) {
					std::cout << "[WARN] SEH handler table detected - exception handling may not work" << std::endl; risks_found++;
				}
			} else {
				if (cfg->SecurityCookie) {
					std::cout << "[WARN] /GS security cookie detected - may fail in no-reloc fallback mode" << std::endl; risks_found++;
				}
				if (cfg->SEHandlerTable && cfg->SEHandlerCount) {
					std::cout << "[WARN] SEH handler table detected - exception handling may not work" << std::endl; risks_found++;
				}
				if (cfg->GuardCFFunctionTable && cfg->GuardCFFunctionCount) {
					std::cout << "[WARN] Control Flow Guard detected - may cause crash in no-reloc mode" << std::endl; risks_found++;
				}
			}
		} else {
			// x86
			auto cfg = (peconv::IMAGE_LOAD_CONFIG_DIR32_W10*)ld_cfg;
			if (cfg->Size >= sizeof(*cfg)) {
				if (cfg->SecurityCookie) {
					std::cout << "[WARN] /GS security cookie detected - may fail in no-reloc fallback mode" << std::endl; risks_found++;
				}
			...
			} else if (cfg->Size >= sizeof(peconv::IMAGE_LOAD_CONFIG_DIR32_W8)) {
				auto cfg8 = (peconv::IMAGE_LOAD_CONFIG_DIR32_W8*)ld_cfg;
				if (cfg8->SecurityCookie) {
					std::cout << "[WARN] /GS security cookie detected - may fail in no-reloc fallback mode" << std::endl; risks_found++;
				}
				if (cfg8->SEHandlerTable && cfg8->SEHandlerCount) {
	...
	{
		size_t dir_size = 0;
		IMAGE_DELAYLOAD_DESCRIPTOR* dl = peconv::get_delayed_imps(my_exe, exe_size, dir_size);
		if (dl && dir_size > 0) {
			std::cout << "[WARN] Delay-load imports detected - may not resolve correctly in no-reloc mode" << std::endl;
			risks_found++;
		}
	}
	...
	//--- Check .bss sections ---
	{
		...
		for (size_t i = 0; i < peconv::get_sections_count(my_exe, exe_size); i++) {
			sec = peconv::get_section_hdr(my_exe, exe_size, i);
			if (sec && sec->SizeOfRawData == 0 && sec->Misc.VirtualSize > 0) {
				has_bss = true;
				break;
			}
		}
		if (has_bss) {
			std::cout << "[WARN] .bss section (uninitialized data) detected - will be zero-filled" << std::endl;
			risks_found++;
		}
	}

	//--- Check SectionAlignment ---
	{
		DWORD sec_align = peconv::get_sec_alignment(my_exe, false);
		if (sec_align > 0x10000) {
			std::cout << "[WARN] Large section alignment (0x"
				<< std::hex << sec_align << std::dec
				<< ") detected - may cause issues with no-reloc loading" << std::endl;
			risks_found++;
		}
	}

	if (risks_found > 0) {
		std::cout << "[INFO] " << risks_found << " risk(s) found - review warnings above." << std::endl;
	}
}
...
bool is_supported_pe(BYTE *my_exe, size_t exe_size)
{
	...
	if (!peconv::has_relocations(my_exe)) {
		std::cout << "[INFO] No relocation table found - using no-reloc loader mode." << std::endl;
		scan_no_reloc_risks(my_exe, exe_size);
	}
